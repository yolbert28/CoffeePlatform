...
data class AddPaymentUiState(
    ...
    val remainingAmount: Double = 0.0, // Para mostrar cuánto falta
    val currency: String = "",
...
class AddPaymentScreenModel(
    ...
    fun savePayment() {
        ...
        if (s.selectedLoanWrapper == null) {
            _state.update { it.copy(error = "Selecciona un préstamo") }
            return
        }
        ...
        if (amountVal > s.remainingAmount + 0.01) { // Pequeño margen
            _state.update { it.copy(error = "El monto excede la deuda restante (${s.remainingAmount})") }
            return
        }
